...
public class CycleDelayFilter
{
  ...
  {
    CycleDelayFilter delay = new CycleDelayFilter(5);
    System.out.format("Cycle %2d: %s -> %s\n", 0, Boolean.toString(false), Boolean.toString(delay.compute(false)));
    for (int cycle = 1; cycle < 10; ++cycle)
      System.out.format("Cycle %2d: %s -> %s\n", cycle, Boolean.toString(true), Boolean.toString(delay.compute(true)));
  }
}